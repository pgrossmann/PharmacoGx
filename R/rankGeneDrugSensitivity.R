...
function (data, drug, drug.id, drug.concentration, type, xp, batch, single.type=FALSE, nthread=1, verbose=FALSE) {
  ...
  for (bb in 1:length(ubatch)) {
    ...
    ctrlix <- rownames(data)[complete.cases(batch, xp) & batch == ubatch[bb] & xp == "control"]
      ...
      inpumat <- rbind(inpumat, data.frame("treated"=c(rep(1, length(xpix)), rep(0, length(ctrlix))), "type"=c(type[xpix], type[ctrlix]), "batch"=paste("batch", c(batch[xpix], batch[ctrlix]), sep=""), "concentration"=c(conc[xpix], conc[ctrlix])))
		}
	}
    ...
    if (nrow(inpumat2) < 3 || length(sort(unique(inpumat2[ , "concentration"]))) < 2) {
      ...
      if(nthread > 1) {
        ...
        mcres <- parallel::mclapply(splitix, function(x, data, inpumat) {
    			res <- t(apply(data[rownames(inpumat), x, drop=FALSE], 2, geneDrugPerturbation, concentration=inpumat[ , "concentration"], type=inpumat[ , "type"], batch=inpumat[ , "batch"]))
          return(res)
